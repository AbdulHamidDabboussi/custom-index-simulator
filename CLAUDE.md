...
```bash
# Build / refresh the offline price bundle (src/data.js). Fetches server-side
...
node src/fetch-data.js                 # all S&P 100 + SPY/QQQ, last ~300 months
node src/fetch-data.js --months=180    # shorter window (~15y)
node src/fetch-data.js --delay=600     # slower pacing if a source throttles
