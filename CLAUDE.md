...
7. **`rebaseToBench`** + **`renderResults`** — benchmarks (SPY, QQQ, SPUS) are rebased to 100 on the same month axis — or, for one that launched mid-window (SPUS), to 100 at its first available month with `null` before it (the chart line starts at inception). Results render as stat cards, a Chart.js line chart, and comparison/composition tables.
